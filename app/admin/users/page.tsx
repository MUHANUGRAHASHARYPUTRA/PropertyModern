        ...
        <header className="flex justify-between items-center mb-12">
          <div>
            <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-serif text-brand-charcoal ">Manajemen <span className="italic text-brand-gold">Pengguna</span></h1>
            </div>
            <p className="text-brand-charcoal/60  text-sm">Lihat data pendaftaran dan kontak WhatsApp calon pembeli.</p>
          </div>
          ...
          <div className="relative">
              ...
              <input 
                ...
                className="pl-12 pr-6 py-3 bg-white  border border-brand-charcoal/5  rounded-full text-xs outline-none focus:ring-1 focus:ring-brand-gold min-w-[300px]"
              />
          </div>
        </header>
                    ...
                    {registrations?.map((reg) => (
                        <tr key={reg.id} className="text-sm group hover:bg-brand-gold/[0.02] transition-colors">
                            <td className="px-8 py-6">
                                <span className="font-medium ">{reg.full_name || 'Anonymous'}</span>
                            </td>
                            <td className="px-8 py-6">
                                <a 
                            ...
                            <td className="px-8 py-6">
                                <span className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase ${reg.role === 'admin' ? 'bg-brand-gold/10 text-brand-gold' : 'bg-brand-charcoal/10 text-brand-charcoal/60  font-medium'}`}>
                                    {reg.role}
                                </span>
                            </td>
