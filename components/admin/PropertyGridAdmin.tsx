  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {properties?.map((prop) => (
          <div key={prop.id} className="bg-white  rounded-[2.5rem] border border-brand-charcoal/5  shadow-sm overflow-hidden group">
            ...
            <div className="p-8">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-serif  leading-tight">{prop.name}</h3>
              ...
              <div className="flex gap-4 mb-8 text-[10px] font-bold uppercase tracking-widest text-brand-charcoal/40 ">
                <span>{prop.luas_bangunan}/{prop.luas_tanah} m²</span>
                <span>{prop.kamar_tidur} Bed</span>
                <span>{prop.kamar_mandi} Bath</span>
              </div>
              
              <div className="flex gap-3 pt-6 border-t border-brand-charcoal/5 ">
                <button 
                  onClick={() => setEditingProperty(prop)}
                  className="flex-1 flex items-center justify-center gap-2 py-3 bg-brand-offwhite  text-brand-charcoal/60  text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-brand-gold hover:text-white transition-all"
                >
                ...
                <Link href={`/#${prop.category}`} className="p-3 bg-brand-offwhite  text-brand-charcoal/30 hover:text-brand-gold rounded-xl transition-all">
        ...
        {!properties?.length && (
          <div className="col-span-full py-20 text-center bg-white  rounded-[2.5rem] border border-dashed border-brand-charcoal/10">
